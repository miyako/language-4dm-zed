...
use tool4d_lsp_stdio::relay::{
    relay_editor_stream, relay_tool4d_stream,
    CompatibilityState,
};
...
fn read_single_frame(input: &[u8]) -> (usize, Vec<u8>) {
    ...
    let header_text =
        std::str::from_utf8(header)
            .expect("LSP headers must be ASCII");
    ...
    assert_eq!(
        input.len(),
        body_end,
        "test expected exactly one LSP frame"
    );

    (
        content_length,
        input[body_start..body_end].to_vec(),
    )
}
...
fn read_single_json_frame(input: &[u8]) -> Value {
    ...
    serde_json::from_slice(&body)
        .expect("frame body must contain JSON")
}
...
fn repairs_null_diagnostic_response_with_numeric_id() {
    ...
    relay_editor(&state, &request)
        .expect("diagnostic request should be relayed");

    let response =
        tool4d_json_frame(&null_response(json!(10)));

    let output = relay_tool4d(&state, &response)
        .expect("diagnostic response should be relayed");
...
fn repairs_null_diagnostic_response_with_string_id() {
    ...
    let request =
        json_frame(&diagnostic_request(json!("diagnostic-11")));

    relay_editor(&state, &request)
        .expect("diagnostic request should be relayed");

    let response =
        tool4d_json_frame(&null_response(json!("diagnostic-11")));
...
fn unrelated_null_response_is_unchanged() {
    ...
    relay_editor(&state, &json_frame(&request))
        .expect("shutdown request should be relayed");
  ...
  "result": null
    ...
    let output = relay_tool4d(
        &state,
        &tool4d_frame(response_body),
    )
...
fn valid_full_diagnostic_report_is_unchanged() {
    ...
    relay_editor(
        &state,
        &json_frame(&diagnostic_request(json!(13))),
    )
    .expect("diagnostic request should be relayed");
...
fn valid_unchanged_diagnostic_report_is_unchanged() {
    ...
    relay_editor(
        &state,
        &json_frame(&diagnostic_request(json!(14))),
    )
    .expect("diagnostic request should be relayed");

    let response_body = br#"{"jsonrpc":"2.0","id":14,"result":{"kind":"unchanged","resultId":"previous-result"}}"#;
...
fn diagnostic_error_response_is_unchanged() {
    ...
    relay_editor(
        &state,
        &json_frame(&diagnostic_request(json!(15))),
    )
    .expect("diagnostic request should be relayed");

    let error_response_body = br#"{"jsonrpc":"2.0","id":15,"error":{"code":-32603,"message":"Internal error"}}"#;

    let first_output = relay_tool4d(
        &state,
        &tool4d_frame(error_response_body),
    )
     ...
     */
    let repeated_body =
        br#"{"jsonrpc":"2.0","id":15,"result":null}"#;

    let second_output = relay_tool4d(
        &state,
        &tool4d_frame(repeated_body),
    )
...
fn handles_out_of_order_diagnostic_responses() {
    ...
    let mut requests =
        json_frame(&diagnostic_request(json!(20)));

    requests.extend_from_slice(
        &json_frame(&diagnostic_request(json!(21))),
    );

    relay_editor(&state, &requests)
        .expect("diagnostic requests should be relayed");

    let response_21 =
        tool4d_json_frame(&null_response(json!(21)));

    let output_21 = relay_tool4d(&state, &response_21)
        .expect("response 21 should be relayed");
    ...
    let response_20 =
        tool4d_json_frame(&null_response(json!(20)));

    let output_20 = relay_tool4d(&state, &response_20)
        .expect("response 20 should be relayed");
...
fn repeated_response_is_not_rewritten() {
    ...
    relay_editor(
        &state,
        &json_frame(&diagnostic_request(json!(22))),
    )
    .expect("diagnostic request should be relayed");

    let first_response =
        tool4d_json_frame(&null_response(json!(22)));

    let first_output =
        relay_tool4d(&state, &first_response)
            .expect("first response should be relayed");
    ...
    let repeated_body =
        br#"{"jsonrpc":"2.0","id":22,"result":null}"#;
...
fn diagnostic_notification_is_not_tracked() {
    ...
    relay_editor(&state, &json_frame(&notification))
        .expect("notification should be relayed");

    let response_body =
        br#"{"jsonrpc":"2.0","id":23,"result":null}"#;

    let output = relay_tool4d(
        &state,
        &tool4d_frame(response_body),
    )
    .expect("response should be relayed");
...
fn malformed_json_payload_is_forwarded_unchanged() {
    ...
    let editor_output = relay_editor(
        &state,
        &frame(malformed_body),
    )
    .expect("malformed editor payload should be relayed");

    let (_, editor_body) =
        read_single_frame(&editor_output);

    assert_eq!(editor_body, malformed_body);

    let tool4d_output = relay_tool4d(
        &state,
        &tool4d_frame(malformed_body),
    )
    .expect("malformed tool4d payload should be relayed");

    let (_, tool4d_body) =
        read_single_frame(&tool4d_output);
...
fn rewritten_content_length_uses_utf8_byte_length() {
    ...
    relay_editor(&state, &json_frame(&request))
        .expect("Unicode request should be relayed");
    ...
    let output = relay_tool4d(
        &state,
        &tool4d_json_frame(&response),
    )
    .expect("Unicode response should be relayed");

    let (content_length, body) =
        read_single_frame(&output);

    assert_eq!(content_length, body.len());

    let value: Value = serde_json::from_slice(&body)
        .expect("rewritten response must be JSON");
...
fn handles_fragmented_request_and_response_reads() {
    ...
    let request =
        json_frame(&diagnostic_request(json!(25)));
    ...
    let response =
        tool4d_json_frame(&null_response(json!(25)));
...
fn transformed_response_has_canonical_header() {
    ...
    relay_editor(
        &state,
        &json_frame(&diagnostic_request(json!(26))),
    )
    .expect("diagnostic request should be relayed");

    let response =
        tool4d_json_frame(&null_response(json!(26)));
    ...
    assert_eq!(
        serde_json::from_slice::<Value>(&body)
            .expect("response should contain JSON")["result"],
        json!({
            "kind": "full",
            "items": []
        })
